...
import { CheckCircle2, Globe2, Loader2, Send, Sparkles, Music2, Video, Camera} from "lucide-react";
          ...
          {PLATFORMS.map((p) => {
            ...
            return (
              <button key={p.id} className={`platform-select-card ${active ? "selected" : ""}`} onClick={() => toggle(p.id)}>
                <div className={`platform-mark ${p.tone}`}>{p.icon ? <p.icon size={18} strokeWidth={1.8} /> : null}</div>
                <div>
